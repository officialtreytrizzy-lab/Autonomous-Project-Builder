...
export function SourceManager({ intakeId, sources, onChanged, onView }: {
  ...
  return <section className="source-manager" aria-labelledby="sources-title">
    ...
    <input ref={replaceInput} hidden type="file" accept={sources.find((source) => source.sourceId === replaceId)?.role === 'implementation-plan' ? '.pdf,.doc,.docx' : '.pdf,.doc,.docx,.txt,.md,.png,.jpg,.jpeg,.webp'} onChange={(event) => void replace(event.target.files?.[0])} />
    <div className="source-lines">{sources.map((source) => <article key={source.sourceId} className={source.availability === 'deleted' ? 'source-line unavailable' : 'source-line'}>
      <span className="source-icon"><FileText size={16} /></span><div><strong>{source.originalFilename}</strong>{source.role === 'implementation-plan' ? <span className="source-role-plan">Implementation plan</span> : null}<small>Revision {source.revision} · {source.inspectedPageCount || 0}/{source.pageCount || ' - '} pages inspected · {source.availability === 'deleted' ? 'original unavailable' : 'retained locally'}</small></div>
      <span className={`source-state source-${source.processingStatus}`}>{busyId === source.sourceId ? 'working' : source.processingStatus}</span>
      <div className="source-actions"><button disabled={source.availability === 'deleted'} onClick={() => onView(source)} aria-label={`View ${source.originalFilename}`}><Eye size={14} /></button><button disabled={source.availability === 'deleted'} onClick={() => { setReplaceId(source.sourceId); if (replaceInput.current) { replaceInput.current.accept = source.role === 'implementation-plan' ? '.pdf,.doc,.docx' : '.pdf,.doc,.docx,.txt,.md,.png,.jpg,.jpeg,.webp'; replaceInput.current.click(); } }} aria-label={`Replace ${source.originalFilename}`}><RefreshCw size={14} /></button><button disabled={source.availability === 'deleted' || source.role === 'implementation-plan'} title={source.role === 'implementation-plan' ? 'Replace the implementation plan instead of deleting the authoritative build source.' : undefined} onClick={() => void remove(source)} aria-label={`Delete ${source.originalFilename}`}><Trash2 size={14} /></button></div>
    </article>)}</div>
  </section>;
}
