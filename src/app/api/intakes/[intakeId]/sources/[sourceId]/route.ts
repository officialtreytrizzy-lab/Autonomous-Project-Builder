...
export async function replaceOrDeleteSourceResponse(request: Request, intakeId: string, sourceId: string, store: IntakeStore) {
  ...
  if (request.method === 'DELETE') {
    if (source.role === 'implementation-plan') return Response.json({ error: 'Replace the implementation plan instead of deleting the authoritative build source' }, { status: 400 });
  ...
  try {
    ...
    if (source.role === 'implementation-plan' && !['.pdf', '.doc', '.docx'].includes(extname(file.name).toLowerCase())) return Response.json({ error: 'Implementation plans can only be replaced with PDF, DOC, or DOCX files' }, { status: 400 });
    const replacement = await replaceSource(sourceId, Readable.from(file.stream() as unknown as AsyncIterable<Uint8Array>), {
      filename: file.name, declaredMimeType: file.type, intakeId, projectWorkspace: project.workspace, store, role: source.role || 'reference',
    });
