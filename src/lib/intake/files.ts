...
import type { SourceRole } from './types.ts';
...
function validateDetectedType(extension: string, detected: { ext: string; mime: string } | undefined, bytes: Buffer, declaredMimeType?: string) {
  ...
  if (extension === '.doc' && detected.mime === 'application/x-cfb' && declaredMimeType === 'application/msword') return detected;
...
export async function storeSource(stream: NodeJS.ReadableStream, context: SourceContext & { sourceId?: string }) {
  ...
  const intakeData = join(context.projectWorkspace, '.builder', 'intake-data');
  const originals = join(intakeData, 'originals');
  const incoming = join(originals, '.incoming');
  mkdirSync(incoming, { recursive: true });
  mkdirSync(join(intakeData, 'derived'), { recursive: true });
  mkdirSync(join(intakeData, 'briefs'), { recursive: true });
  mkdirSync(join(intakeData, 'evidence'), { recursive: true });
