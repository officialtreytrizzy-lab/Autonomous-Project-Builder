...
import { geminiApiKey } from '../ai/gemini.ts';
import { getSecureVault, secretFingerprint } from '../secure-vault.ts';
...
async function ensureGeminiProviderVault() {
  const value = geminiApiKey();
  const vaultPath = process.env.BUILDER_SECRET_VAULT?.trim() || '';
  if (!value || !vaultPath) return '';
  const vault = getSecureVault();
  const fingerprint = secretFingerprint(value);
  if (vault.metadata('provider.gemini.api-key')?.fingerprint !== fingerprint) {
    await vault.set({ key: 'provider.gemini.api-key', label: 'Gemini API key', provider: 'google-gemini', value });
  }
  return vaultPath;
}
...
export function prepareIntakeWorkerLaunch(input: {
  ...
  const secretVault = input.secretVaultPath ? ` --secret-vault '${escapePowerShell(input.secretVaultPath)}'` : '';
  const visionModel = input.visionModel ? ` --vision-model '${escapePowerShell(input.visionModel)}'` : '';
...
try {
  ...
  & $node '${escapePowerShell(input.workerPath)}' --database '${escapePowerShell(input.databasePath)}' --intake '${escapePowerShell(input.intakeId)}'${renderer}${secretVault}${visionModel} 1> $stdout 2> $stderr
...
export class IntakeService {
  ...
  async analyze(intakeId: string) {
    ...
    const hasImplementationPlan = this.store.currentSources(intake.id).some((source) => source.availability === 'available' && source.role === 'implementation-plan');
    if (project.inputMode === 'implementation-plan' && !hasImplementationPlan) throw new Error('Upload the implementation plan before Builder analyzes this project');
    ...
    if (!planId) {
      const plan = await this.caller('plan_create', {
        goal: hasImplementationPlan
          ? `Read the imported implementation plan for ${project.name} as the authoritative build contract, inspect every retained supporting source, and synthesize a source-grounded Build Brief without allowing supporting files to silently override the plan. Every visual page must be inspected before approval.`
          : `Understand all locally retained source evidence for ${project.name} and synthesize a source-grounded Build Brief. Every visual page must be inspected before approval.`,
    ...
    const secretVaultPath = await ensureGeminiProviderVault();
    const launch = prepareIntakeWorkerLaunch({
      ...
      secretVaultPath,
      visionModel: process.env.BUILDER_VISION_MODEL?.trim() || 'gemini-3.7-flash',
    });
