import { targetIsWebRuntime, targetLabel, targetNeedsAndroidToolchain, targetNeedsAppleBuildHost, type BuildTargetSelection } from './target-platform.ts';
...
export function analyzeIngredients(input: BuildRequest): Ingredient[] {
  ...
  ingredients.push(ingredient({ id: 'target', label: 'Build Target', level: 'green', required: true, available: true, target: 'computer-2', detail: targetLabel(input.target) }));
  if (targetNeedsAndroidToolchain(input.target)) ingredients.push(ingredient({ id: 'android-toolchain', label: 'Android Toolchain', level: 'yellow', required: true, available: true, target: 'computer-2', detail: 'Verify or install the Android SDK, Gradle, and packaging tools before producing the requested APK/AAB artifact.' }));
  if (targetNeedsAppleBuildHost(input.target)) ingredients.push(ingredient({ id: 'apple-build-host', label: 'Apple Build Host', level: 'yellow', required: true, available: true, target: 'cloud', detail: 'Apple-native packaging requires a macOS/Xcode build environment. Reuse an existing configured Apple build resource before escalating.' }));
...
export function buildExecutionPlan(input: BuildRequest, ingredients = analyzeIngredients(input)): ExecutionStep[] {
  ...
  const steps: ExecutionStep[] = [
    { id: 'target-toolchain', title: `Prepare ${targetLabel(input.target)} toolchain`, target: targetNeedsAppleBuildHost(input.target) ? 'cloud' : 'computer-2', status: blocked ? 'blocked' : 'ready', reason: 'The selected device/runtime/deliverable controls scaffolding, packaging, signing, and verification.' },
  ...
  else if (targetIsWebRuntime(input.target)) steps.push({ id: 'runtime', title: 'Launch and verify the private local application', target: 'computer-2', status: blocked ? 'blocked' : 'ready', reason: 'The selected web deliverable is verified against its real local runtime.' });
  else steps.push({ id: 'package', title: `Package and verify ${targetLabel(input.target)}`, target: targetNeedsAppleBuildHost(input.target) ? 'cloud' : 'computer-2', status: blocked ? 'blocked' : 'ready', reason: 'Produce the requested installable artifact and verify that artifact rather than substituting a web build.' });
  return steps;
}
