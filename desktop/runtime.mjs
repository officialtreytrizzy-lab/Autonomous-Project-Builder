...
export const ALLOWED_SERVER_ENVIRONMENT = new Set([
  ...
  'BUILDER_STATE_DB',
  'BUILDER_HARD_MODEL_ENDPOINT',
  'BUILDER_HARD_MODEL',
  'BUILDER_QWEN_MULTIMODAL_ENDPOINT',
  'BUILDER_MULTIMODAL_MODEL',
  'BUILDER_VISION_MODEL',
  'BUILDER_NEEDLE_ENDPOINT',
  'BUILDER_DESIGN_MODEL',
  'BUILDER_CODING_BACKUP_MODEL',
  'BUILDER_FREE_REASONING_MODEL',
  'BUILDER_DESIGN_FALLBACK_MODEL',
  'BUILDER_DESIGN_IMAGE_PROVIDER',
  'BUILDER_DESIGN_IMAGE_MODEL',
  'BUILDER_DESIGN_IMAGE_FALLBACK',
  'BUILDER_DESIGN_IMAGE_QUALITY_MODEL',
  'BUILDER_DESIGN_IMAGE_SIZE',
  'BUILDER_DESIGN_QA_THRESHOLD',
  'BUILDER_DESIGN_QA_REPAIR_ROUNDS',
  'GEMINI_API_KEY',
  'GOOGLE_API_KEY',
...
export const PERSISTENT_CONFIG_OVERRIDE_KEYS = new Set([
  'BUILDER_DESIGN_MODEL',
  'BUILDER_DESIGN_FALLBACK_MODEL',
  'BUILDER_DESIGN_IMAGE_PROVIDER',
  'BUILDER_DESIGN_IMAGE_MODEL',
...
export function resolveDesktopPaths({ isPackaged, resourcesPath, userDataPath, cwd, homeDirectory }) {
  ...
  return {
    userDataPath,
    serverPath: isPackaged ? join(resourcesPath, 'builder', 'server.js') : join(cwd, '.next_build', 'standalone', 'server.js'),
    ...
    secretVault: join(stateDirectory, 'secure-credentials.json'),
    projectsRoot: homeDirectory,
...
export function buildServerLaunch({
  ...
  return {
    ...
    env: {
      ...
      ...(secretVault ? { BUILDER_SECRET_VAULT: secretVault } : {}),
...
export async function discoverComputer2Environment({
    ...
    for (const filename of ['.env.local', '.env.mcp']) {
      try {
        ...
        for (const [key, value] of Object.entries(parsed)) {
          if (PERSISTENT_CONFIG_OVERRIDE_KEYS.has(key) || !discovered[key]) discovered[key] = value;
        }
      } catch {
  ...
  if (!discovered.BUILDER_NEEDLE_ENDPOINT) {
    discovered.BUILDER_NEEDLE_ENDPOINT = 'http://127.0.0.1:3322';
  }
