...
test('durable agent control leases ownership and retains verification evidence', () => withStore((store) => {
  const build = store.create({ request: { objective: 'Ship safely' }, analysis: {}, workspace: 'control' });
  const claimed = store.claimControl(build.id, 'worker-a', 60_000);
  assert.equal(claimed.owner, 'worker-a');
  assert.throws(() => store.claimControl(build.id, 'worker-b', 60_000), /leased by worker-a/);
  const evidenced = store.recordEvidence(build.id, { kind: 'http', summary: '127.0.0.1 responded 200', ref: 'http-check' });
  assert.equal(evidenced.evidence.at(-1).kind, 'http');
  store.update(build.id, { currentStage: 'verification', currentStep: 'Run visual QA' });
  assert.equal(store.get(build.id).control.currentGate, 'verification');
  assert.deepEqual(store.get(build.id).control.todos, ['Run visual QA']);
  const released = store.releaseControl(build.id, 'worker-a');
