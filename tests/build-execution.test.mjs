...
test('execution preparation keeps the goal in a file rather than the shell command', () => {
  ...
  try {
    ...
    assert.match(prompt, /connected Computer 2 MCP/);
    assert.match(prompt, /approved-requirements\.json/);
    assert.match(prompt, /run-with-secrets\.ps1/);
    assert.match(prompt, /Never open, decode, print, copy, or echo \.builder\/runtime-secrets\.json/i);
    ...
    assert.match(bundledWorker, /key: 'Enter'/);
    assert.match(bundledWorker, /if \(composer\.box\) return/);
    assert.doesNotMatch(bundledWorker, /composer\.box && composer\.send/);
...
test('execution can attach an existing application workspace without scaffolding over it', () => {
  const root = mkdtempSync(join(tmpdir(), 'builder-existing-'));
  const existing = join(root, 'current-app');
  try {
    mkdirSync(existing, { recursive: true });
    writeFileSync(join(existing, 'package.json'), '{"name":"current-app"}');
    const prepared = prepareBuildWorkspace({
      root: join(root, 'managed-projects'),
      workspace: existing,
      existingWorkspace: true,
      buildId: 'build-existing-1',
      port: 3242,
      request: { name: 'Current App', objective: 'Add a real feature without rebuilding the app', deployment: 'local', backend: 'none', workflow: 'none' },
      steps: [{ id: 'verify', title: 'Verify', target: 'computer-2', status: 'ready', reason: 'Gate' }],
    });
    assert.equal(prepared.workspace, existing);
    assert.equal(readFileSync(join(existing, 'package.json'), 'utf8'), '{"name":"current-app"}');
    assert.match(readFileSync(prepared.promptPath, 'utf8'), /existing application or repository selected by the user/i);
    assert.match(readFileSync(prepared.promptPath, 'utf8'), /modify it in place/i);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
