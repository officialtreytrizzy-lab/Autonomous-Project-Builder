...
test('low Kimi design score invalidates completion and queues a bounded visual repair', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'builder-design-repair-'));
  const store = new BuildStore(join(directory, 'build-state.db'));
  const intakeStore = new IntakeStore(join(directory, 'intake-state.db'));
  const calls = [];
  let jobStatus = { job_id: 'job-design-1', status: 'running', attempt: 1, retryCount: 0 };
  const caller = async (tool, args) => {
    calls.push({ tool, args });
    if (tool === 'plan_create') return { id: 'plan-design-1' };
    if (tool === 'job_submit') return { job_id: `job-design-${calls.filter((call) => call.tool === 'job_submit').length}`, status: 'queued' };
    if (tool === 'job_status') return jobStatus;
    if (tool === 'job_result') return { job_id: jobStatus.job_id, status: jobStatus.status, result: { captured: true } };
    throw new Error(`Unexpected tool ${tool}`);
  };
  const projectWorkspace = join(directory, 'project');
  const project = intakeStore.createProject({ name: 'Visual Repair', objective: 'Ship the approved interface', workspace: projectWorkspace });
  const intake = intakeStore.createIntake(project.id);
  intakeStore.createBriefVersion(intake.id, {
    outcome: 'Ship the approved interface', users: ['Owner'], flows: ['Open dashboard'], requirements: ['Responsive'],
    designDirection: ['Quiet luxury'], dataAndIntegrations: [], exclusions: [], acceptanceTests: ['Dashboard is usable'], assumptions: [],
  }, { inspectedPages: 0, totalPages: 0, complete: true });
  const model = 'test-kimi';
  intakeStore.ensureDesignSession(intake.id, model);
  intakeStore.appendDesignMessage(intake.id, { role: 'assistant', content: 'Approved visual direction.', model });
  const design = intakeStore.saveDesignContract({
    id: 'design-repair-1', intakeId: intake.id, projectId: project.id, version: 1, status: 'approved', provider: 'openrouter', model,
    approvedAt: new Date().toISOString(), summary: 'Quiet luxury dashboard', principles: ['Clear hierarchy'],
    designSystem: { visualLanguage: 'Quiet luxury', typography: ['Readable sans'], colorAndMaterial: ['Dark glass'], spacingAndShape: ['12px rhythm'], elevationAndDepth: ['Subtle depth'], motion: ['Short transitions'] },
    screens: [], interactions: [], responsiveRules: [], accessibility: [], assets: [], implementationRules: ['Match exactly'], visualAcceptance: ['Desktop and mobile match'],
  });
  const service = new BuildService({
    store, intakeStore, callComputer2: caller, projectsRoot: join(directory, 'projects'), allocatePort: async () => 3247,
    designQaConfigured: () => true,
    reviewDesign: async (_intakeId, screenshots, threshold) => ({
      id: 'qa-94', intakeId: intake.id, projectId: project.id, designId: design.id, model, score: 94, threshold: threshold || 98, passed: false,
      summary: 'Mobile hierarchy drifts from the approved design.', strengths: ['Materials match'],
      mismatches: [{ area: 'Mobile header', severity: 'high', expected: 'Compact header', observed: 'Oversized header', repair: 'Reduce header height and type scale' }],
      screenshots: screenshots.map((shot) => shot.label), createdAt: new Date().toISOString(),
    }),
  });
  try {
    const build = await service.start({ name: 'Visual Repair', objective: 'Ship the approved interface', deployment: 'local', backend: 'none', workflow: 'none' });
    store.update(build.id, { projectId: project.id, intakeId: intake.id });
    const desktop = join(build.workspace, '.builder', 'visual-qa', 'desktop.png');
    const mobile = join(build.workspace, '.builder', 'visual-qa', 'mobile.png');
    const fs = await import('node:fs');
    fs.mkdirSync(join(build.workspace, '.builder', 'visual-qa'), { recursive: true });
    writeFileSync(desktop, Buffer.from([137, 80, 78, 71]));
    writeFileSync(mobile, Buffer.from([137, 80, 78, 71]));
    const completion = join(build.workspace, '.builder', 'completion.json');
    writeFileSync(completion, JSON.stringify({
      status: 'complete', appUrl: 'http://127.0.0.1:3247', verification: passedChecks, repairs: [],
      screenshots: [{ label: 'desktop', path: desktop, mimeType: 'image/png' }, { label: 'mobile', path: mobile, mimeType: 'image/png' }],
      result: { summary: 'functionally complete' },
    }));
    jobStatus = { job_id: build.jobId, status: 'succeeded', attempt: 2, retryCount: 0 };
    const repaired = await service.refresh(build.id);
    assert.equal(repaired.status, 'queued');
    assert.equal(repaired.currentStage, 'visual-qa');
    assert.match(repaired.currentStep, /Visual design repair round 1 queued/);
    assert.equal(repaired.designQaAttempts, 1);
    assert.equal(repaired.designQa?.score, 94);
    assert.equal(existsSync(completion), false);
    assert.equal(existsSync(join(build.workspace, '.builder', 'design-qa.json')), true);
    assert.equal(calls.filter((call) => call.tool === 'job_submit').length, 2);
  } finally {
    store.close(); intakeStore.close(); rmSync(directory, { recursive: true, force: true });
  }
});
